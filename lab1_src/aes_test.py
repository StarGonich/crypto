...
def plot_histograms(original, encrypted, image_name, output_dir):
    ...
    for i in range(3):
        ...
        axes[1, i].set_title(f'{channel_names[i]} канал (aes-{MODE})')
        axes[1, i].set_xlim(0, 255)
        axes[1, i].grid(True, alpha=0.3)

    plt.suptitle(f'Гистограммы: {image_name} - aes-{MODE}', fontsize=14)
    plt.tight_layout()
...
def run_encryption_decryption_direct(input_file, output_dir, key):
    ...
    encrypted_bytes = aes.aes_encrypt(MODE, original_bytes, key, iv)
    decrypted_bytes = aes.aes_decrypt(MODE, encrypted_bytes, key)
    encrypted_fixbytes = aes.fix_bytes(encrypted_bytes, original_shape)
    decrypted_fixbytes = aes.fix_bytes(decrypted_bytes, original_shape)
    encrypted_img = bytes_to_image(encrypted_fixbytes, original_shape)
    decrypted_img = bytes_to_image(decrypted_fixbytes, original_shape)
...
def test_key_sensitivity(original_img, key):
    ...
    encrypted_bytes1 = aes.aes_encrypt(MODE, original_bytes, key, iv)
    encrypted_fixbytes1 = aes.fix_bytes(encrypted_bytes1, original_shape)
    encrypted_img1 = bytes_to_image(encrypted_fixbytes1, original_shape)

    key2 = key[:-1] + bytes([key[-1] ^ 1])

    encrypted_bytes2 = aes.aes_encrypt(MODE, original_bytes, key2, iv)
    encrypted_fixbytes2 = aes.fix_bytes(encrypted_bytes2, original_shape)
    encrypted_img2 = bytes_to_image(encrypted_fixbytes2, original_shape)
...
def test_key_avalanche(original_img, key):
    ...
    if MODE == "cbc":
        ...
        iv = os.urandom(8)
    encrypted_bytes1 = aes.aes_encrypt(MODE, original_bytes, key, iv)
    encrypted_fixbytes1 = aes.fix_bytes(encrypted_bytes1, original_img.shape)

    key_modified = key[:-1] + bytes([key[-1] ^ 1])

    encrypted_bytes2 = aes.aes_encrypt(MODE, original_bytes, key_modified, iv)
    encrypted_fixbytes2 = aes.fix_bytes(encrypted_bytes2, original_img.shape)

    total_bits = len(encrypted_fixbytes1) * 8
    changed_bits = 0

    for b1, b2 in zip(encrypted_fixbytes1, encrypted_fixbytes2):
        xor_result = b1 ^ b2
...
if __name__ == '__main__':
    ...
    results_dir = os.path.join(current_dir, f"../results/aes_{MODE}_results")
